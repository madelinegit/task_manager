...
# ---------- SOCIAL MEDIA ----------
with tab1:
    st.header("Enterprise Host Options")
...
# ---------- CHARACTER BIO ----------
with tab2:
    st.header("Requirements, cautions")
...
with tab5:
    ...
    FILE = "next_step.csv"
    data = load_data(FILE, ["Step"])

    step = st.text_input("What is the next physical step?")

    if st.button("Save Step"):
        new_row = pd.DataFrame([[step]], columns=["Step"])
        data = pd.concat([data, new_row], ignore_index=True)
        save_data(data, FILE)
        st.success("Saved")

    st.subheader("Saved Steps")
    st.dataframe(data)
